...
% FD-6 method (automatically satisfies frictionless BC), real world background viscosity, diffusivity, 1nd scan
...
for profile=1:length(s)
    ...
    if ~isnan(GR(profile))
        ...
        print('-djpeg',[mdirec strcat('codes_new/', filename(filenum).name(1:end-4), '_ex5_1_',num2str(profile))])
        % plot w
        figure(profile+50)
        [Ph]=SI_pic_2(WFGM,sqrt(KFGM(profile,1)^2+KFGM(profile,2)^2),zz,CL(profile,:));
        print('-djpeg',[mdirec strcat('codes_new/', filename(filenum).name(1:end-4), '_ex5_2_',num2str(profile))])
    end
    % plot profiles
    figure(profile+100)
    DP_pic(v,vz,vzz,b,n2,Ri_r,zz)
    set(gcf, 'PaperPosition',[0 0 8 4]);
    print('-djpeg',[mdirec strcat('codes_new/', filename(filenum).name(1:end-4), '_ex5_3_',num2str(profile))])
...
LON = mlon; LAT = mlat;
eval(strcat('save TG_SI_',  filename(filenum).name(1:end-4), '_ex5.mat II GR CR CI CL KFGM K L zw W V Vz Vzz B N2 Ri LON LAT botz GR_all CR_all CL_all ERR_K_all ERR_B_all'))
end

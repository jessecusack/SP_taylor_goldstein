...
%% also kappa and nu is one-tenth of the Thorpe-scale estimate
%
% INPUTS:
...
% load data
filename=[dir(strcat(datadirec,'T2*')) dir(strcat(datadirec,'T4*')) dir(strcat(datadirec,'T6*'))];
% indexes_T2 = 19:67;
% indexes_T4 = [19:67 172:273];
% indexes_T6 = 1:86;
...
% for profile=eval(strcat('indexes_',filename(filenum).name(1:end-4)))
    ...
    if ~isempty(data.v) && data.z(1)-data.z(end)>10*dz % works with profile longer than 10*dz
        ...
        Kv = interp1(z(:,profile),kappa,zz)./10; %1/10 kappa
        Kv(find(Kv<1e-6)) = nan;
        ...
        if ~isempty(GR_family)
            ...
            CL_family(find(GR_family<0)) = nan;
